...
int main()
{
	...
	//iteration through the picture pixels
	int x = (0 - picture.width() / 2);
	int z = (0 - picture.height() / 2);
	for (CImg<float>::iterator i = picture.begin(); i < picture.end(); i++){
		pictureData.emplace_back(glm::vec3(x++, *i, z));
		if (x == picture.width()) {
			x = (0 - picture.width() / 2);
			z += 1;
		}
